...
class CrossAttention(nn.Module):
    ...
    def forward(self, x1, x2, mask=None):
        # x1 is substrate as query, x2 is product as key and value
        x1 = x1.unsqueeze(1)  # (batch_size, 1, in_dim)
        x2 = x2.unsqueeze(1)  # (batch_size, 1, in_dim)
        batch_size, seq_len1, in_dim1 = x1.size()
        seq_len2 = x2.size(1)

        # 计算 q1, k2, v2，形状为 (batch_size, num_heads, seq_len1 or seq_len2, k_dim or v_dim)
...
class Predict_transformer_cross_attention(torch.nn.Module):
    def __init__(self,params):
        ...
        self.cross_ori_dim = CrossAttention(in_dim=params['latent_dim'], num_heads=params['num_head1'], k_dim=params['hidden_dim1'], v_dim=params['hidden_dim1'])
        self.cross_dim_ori = CrossAttention(in_dim=params['latent_dim'], num_heads=params['num_head1'], k_dim=params['hidden_dim2'], v_dim=params['hidden_dim2'])
