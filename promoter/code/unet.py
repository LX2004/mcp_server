...
class PositionalEmbedding(nn.Module):
    """Computes a positional embedding of timesteps.
...
class Downsample(nn.Module):
    """Strided convolution block (shape checks retained)."""
...
class Upsample(nn.Module):
    """Resize-convolution style block (no explicit interpolation)."""
...
class AttentionBlock(nn.Module):
    """QKV self-attention with residual connection."""
...
class ResidualBlock(nn.Module):
    """Two conv blocks with residual connection (+ optional time/class conditioning and attention)."""
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.init_conv = nn.Conv2d(
            img_channels, base_channels, (1, out_init_conv_padding * 2 + 1), padding=(0, out_init_conv_padding)
        )

        self.out_norm = get_norm(norm, base_channels, num_groups)
        self.out_conv = nn.Conv2d(
            base_channels, img_channels, (1, out_init_conv_padding * 2 + 1), padding=(0, out_init_conv_padding)
        )
