...
def train(params, features_array, structure_array, labels_array):
   ...
   # 循环遍历每个折叠
    for fold, (train_indices, val_indices) in enumerate(kf.split(features_array)):
        ...
        if loss_kind == 'pearson':
            write_good_record(dict1=params,dict2=dict2,file_path='../result/on_good_record_metric_pearson_ori_dim_structure.txt')# return  min(loss_test)
        
        elif loss_kind == 'pearson_mse':
            write_good_record(dict1=params,dict2=dict2,file_path='../result/on_good_record_metric_pearson_mse_ori_dim_structure.txt')# return  min(loss_test)
        
        elif loss_kind == 'mse':
            write_good_record(dict1=params,dict2=dict2,file_path='../result/on_good_record_metric_mse_ori_dim_structure.txt')# return  min(loss_test)
