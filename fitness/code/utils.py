...
def encode_essential(essential):
    ...
    base_choice = ['NA', 'FALSE', 'TRUE', True, False]
    ...
    if essential in base_choice:
        ...
        if essential == 'FALSE' or essential is False:
            return np.array([0, 1, 0])

        if essential == 'TRUE' or essential is True:
            return np.array([0, 0, 1])

    else:
        raise ValueError(
            f"The input Essential string '{essential}' is not in the allowed list and cannot be one-hot encoded."
        )
...
def encode_ori(ori):
    ...
    if ori in base_choice:
        ...
        raise ValueError(
            f"The input Ori string '{ori}' is not in the allowed list and cannot be one-hot encoded."
        )
...
def encode_coding(coding):
    ...
    if coding in base_choice:
        ...
        if coding == 'FALSE' or coding is False:
            return np.array([0, 1, 0])

        if coding == 'TRUE' or coding is True:
            return np.array([0, 0, 1])

    else:
        raise ValueError(
            f"The input Coding string '{coding}' is not in the allowed list and cannot be one-hot encoded."
        )
...
def trans_output_to_input(fake_im):
    # Encode generated data into evaluator input: convert to sequence then to numpy array
    sample_seq = []
...
def compute_correlation_coefficient(output, label):
    ...
    if has_nan:
        print("NaN values exist in the arrays")

    if np.std(prediction) == 0:
        print('Prediction data has no variance')
        return 0

    if np.std(target) == 0:
        print('Target data has no variance')
        return 0
