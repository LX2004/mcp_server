...
def encode_essential(essential):
    ...
    if essential in base_choice:
        ...
        if essential == 'FALSE' or essential == False:
            return np.array([0,1,0])
        
        if essential == 'TRUE' or essential == True:
            return np.array([0,0,1])
        
        # print("独热向量编码结果:", encoded_vector)
    else:
        # print("输入的字符串不在给定的列表中，无法进行独热向量编码。")
        raise ValueError(f"The input Essential string '{essential}' is not in the allowed list and cannot be one-hot encoded.")
...
def encode_coding(coding):
    ...
    if coding in base_choice:
        ...
        if coding == 'FALSE' or coding == False:
            return np.array([0,1,0])
        
        if coding == 'TRUE' or coding == True:
            return np.array([0,0,1])
        
        # print("独热向量编码结果:", encoded_vector)
    else:
        # 抛出异常
        # raise ValueError(f"输入的字符串 '{coding}' 不在给定的列表中，无法进行独热向量编码。")
        raise ValueError(f"The input Coding string '{coding}' is not in the allowed list and cannot be one-hot encoded.")
